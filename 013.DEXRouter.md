...
![image-20231105215835589](./assets/image-20231105215835589.png)

## 攻击过程

攻击过程非常简单，就调用了`update()`和`functionCallWithValue()`

![image-20231105220023234](./assets/image-20231105220023234.png)

## 攻击详细分析

此次攻击中使得slot 1、2、3、4、发生了变化，很明显是`update()`导致的。成功调用`update()`会使得这几个storage值发生变化，攻击者将其修改为攻击合约地址，为后续调用`functionCallWithValue()`做准备。

![image-20231105220931357](./assets/image-20231105220931357.png)
